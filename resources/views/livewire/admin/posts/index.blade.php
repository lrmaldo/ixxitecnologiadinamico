<div class="p-4 sm:p-6 space-y-8">
    ...
    <div class="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-2xl font-semibold tracking-tight text-zinc-800 dark:text-zinc-100">Publicaciones</h1>
            <p class="text-sm text-zinc-500 dark:text-zinc-400 mt-1">Gestión de contenidos del blog</p>
        </div>
        <div class="flex items-center gap-3 flex-wrap">
            <a href="{{ route('admin.posts.create') }}" class="inline-flex items-center gap-2 rounded-lg bg-gradient-to-tr from-[#021869] to-[#14317F] hover:from-[#021869]/90 hover:to-[#14317F]/90 px-4 py-2.5 text-sm font-medium text-white shadow-sm transition-colors">
                <flux:icon name="plus" class="h-4 w-4" /> Nueva
            </a>
        </div>
    </div>
    ...
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        <div class="sm:col-span-2">
            ...
            <div class="relative">
                ...
                <input id="search" type="text" wire:model.live.debounce.400ms="search" placeholder="Buscar por título..." class="w-full rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-900 pl-9 pr-3 py-2.5 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/30" />
            </div>
        </div>
        <div>
            <label class="sr-only" for="type">Tipo</label>
            <select id="type" wire:model.live="type" class="w-full rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-900 px-3 py-2.5 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/30">
        ...
        <div>
            <label class="sr-only" for="published">Publicado</label>
            <select id="published" wire:model.live="published" class="w-full rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-900 px-3 py-2.5 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/30">
        ...
        <div>
            ...
            <select id="perPage" wire:model.live="perPage" class="w-full rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-900 px-3 py-2.5 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/30">
    ...
    <div class="overflow-hidden rounded-xl border border-zinc-200/70 dark:border-zinc-700/60 bg-white dark:bg-zinc-900/60 backdrop-blur supports-[backdrop-filter]:bg-white/70 dark:supports-[backdrop-filter]:bg-zinc-900/50 shadow-sm">
            ...
            <table class="w-full min-w-[700px] text-sm">
                <thead>
                    <tr class="bg-zinc-50 dark:bg-zinc-800/70 text-xs uppercase tracking-wide text-zinc-500 dark:text-zinc-400">
                        ...
                        <th class="px-4 py-3 text-left font-medium">
                            <button type="button" wire:click="sortBy('type')" class="inline-flex items-center gap-1 group">
                                <span>Tipo</span>
                                <span class="text-zinc-400 group-hover:text-zinc-600 dark:group-hover:text-zinc-300">
                                    @if($sortField === 'type')
                                        <flux:icon name="chevron-{{ $sortDirection === 'asc' ? 'up' : 'down' }}" class="h-3.5 w-3.5" />
                        ...
                        <th class="px-4 py-3 text-left font-medium">
                            <button type="button" wire:click="sortBy('is_published')" class="inline-flex items-center gap-1 group">
                                <span>Publicado</span>
                                <span class="text-zinc-400 group-hover:text-zinc-600 dark:group-hover:text-zinc-300">
                        ...
                        <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/40 transition-colors">
                            <td class="px-4 py-3">
                                <div class="flex flex-col">
                                    <span class="font-medium text-zinc-800 dark:text-zinc-100">{{ $post->title }}</span>
                                    <span class="text-[11px] text-zinc-500 dark:text-zinc-400 mt-0.5">ID #{{ $post->id }}</span>
                                </div>
                            </td>
                            <td class="px-4 py-3">
                                <span class="inline-flex items-center rounded-full bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 text-[11px] font-medium text-zinc-600 dark:text-zinc-300">{{ $post->type }}</span>
                            </td>
                            <td class="px-4 py-3">
                                <span class="inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium {{ $post->is_published ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-300' : 'bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300' }}">
                                    {{ $post->is_published ? 'Sí' : 'No' }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-zinc-500 dark:text-zinc-400 text-xs">{{ $post->created_at?->format('d/m/Y') }}</td>
                            <td class="px-4 py-3 text-right space-x-3">
                                <a href="{{ route('admin.posts.edit', $post->id) }}" class="text-indigo-600 dark:text-indigo-400 hover:underline font-medium">Editar</a>
                                <button type="button" wire:click="confirmDelete({{ $post->id }})" class="text-red-600 hover:text-red-500 font-medium">Eliminar</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-10 text-center text-sm text-zinc-500 dark:text-zinc-400">No hay publicaciones registradas.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="border-t border-zinc-200 dark:border-zinc-700/60 bg-zinc-50/60 dark:bg-zinc-800/40 px-4 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="text-xs text-zinc-500 dark:text-zinc-400">Mostrando {{ $posts->firstItem() }}-{{ $posts->lastItem() }} de {{ $posts->total() }}</div>
            ...
            <div class="fixed inset-0 z-40 flex items-end sm:items-center justify-center">
                ...
                <div class="relative w-full sm:max-w-md mx-auto bg-white dark:bg-zinc-900 rounded-xl shadow-xl border border-zinc-200 dark:border-zinc-700 p-6 m-4 animate-fade-in-up">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1">
                            <h3 class="text-base font-semibold text-zinc-800 dark:text-zinc-100">Eliminar publicación</h3>
