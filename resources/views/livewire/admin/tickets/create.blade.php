    ...
    <form wire:submit.prevent="save" class="space-y-4 bg-white p-6 rounded-2xl shadow transition hover:shadow-lg" x-data="{ open: false }" @click.outside="open=false">
        ...
        <div class="relative">
            <div class="flex items-end justify-between">
                ...
                <button type="button" class="text-sm text-[#d9491e] hover:underline" wire:click="$set('newClienteOpen', true)">Nuevo cliente</button>
            </div>
            <div class="mt-1">
                <input type="text" wire:model.debounce.250ms="clienteSearch" @focus="open=true" @keydown.escape="open=false" placeholder="Buscar por nombre o email" class="w-full rounded-2xl border-gray-300 focus:ring-[#d9491e]" />
                ...
                <div class="mt-3 p-4 border rounded-2xl bg-gray-50">
                    <div class="grid md:grid-cols-3 gap-3">
                        <div>
                            ...
                            <input type="text" wire:model.defer="newClienteNombre" class="mt-1 w-full rounded-2xl border-gray-300 focus:ring-[#d9491e]" />
                            @error('newClienteNombre') <div class="text-sm text-red-600 mt-1">{{ $message }}</div> @enderror
                        </div>
                        <div>
                            <label class="block text-sm">Email</label>
                            <input type="email" wire:model.defer="newClienteEmail" class="mt-1 w-full rounded-2xl border-gray-300 focus:ring-[#d9491e]" />
                            @error('newClienteEmail') <div class="text-sm text-red-600 mt-1">{{ $message }}</div> @enderror
                        </div>
                        <div>
                            <label class="block text-sm">Teléfono</label>
                            <input type="text" wire:model.defer="newClienteTelefono" class="mt-1 w-full rounded-2xl border-gray-300 focus:ring-[#d9491e]" />
                    ...
                    <div class="mt-3 flex justify-end gap-2">
                        ...
                        <button type="button" class="px-3 py-2 bg-[#d9491e] text-white rounded-2xl shadow" wire:click="createCliente">Crear y seleccionar</button>
                    </div>
                </div>
            @endif
        </div>
        ...
        <div>
            ...
            <input type="text" wire:model="titulo" class="mt-1 w-full rounded-2xl border-gray-300 focus:ring-[#d9491e]" />
            @error('titulo') <div class="text-sm text-red-600 mt-1">{{ $message }}</div> @enderror
        </div>

        <div>
            <label class="block text-sm font-medium">Descripción</label>
            <textarea rows="5" wire:model="descripcion" class="mt-1 w-full rounded-2xl border-gray-300 focus:ring-[#d9491e]"></textarea>
        ...
        <div>
            ...
            <select wire:model="prioridad" class="mt-1 w-full rounded-2xl border-gray-300 focus:ring-[#d9491e]">
        ...
        <div class="flex justify-end gap-2">
            ...
            <button type="submit" class="px-4 py-2 bg-[#d9491e] text-white rounded-2xl shadow hover:opacity-90">Crear</button>
        </div>
    </form>
</div>
